...
interface CartPageProps {
  cartItems: CartItem[];
  onUpdateQuantity: (productId: number, size: string, color: string, qty: number) => void;
...
export function CartPage({ cartItems, onUpdateQuantity, onRemoveItem, onClearCart, currency, onNavigate, currentUser }: CartPageProps) {
  ...
  if (cartItems.length === 0) {
    return (
      <div style={{ backgroundColor: 'var(--background)', minHeight: '100vh' }} className="pt-16 flex items-center justify-center px-4">
        <div style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }}
          ...
          <div>
            <h2 style={{ fontFamily: 'var(--font-display)', color: 'var(--foreground)' }} className="text-2xl font-bold uppercase mb-2">Cart is empty</h2>
            <p style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-body)' }} className="text-sm">Browse the shop and add items to get started.</p>
          </div>
      ...
      <div style={{ backgroundColor: 'var(--card)', borderBottom: '1px solid var(--border)' }} className="px-6 py-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <button onClick={onClearCart}
            style={{ color: 'var(--muted-foreground)', border: '1px solid var(--border)', fontFamily: 'var(--font-body)' }}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
            ...
            <div key={`${product.id}-${size}-${color}`}
              ...
              <div className="flex-1 flex flex-col justify-between min-w-0">
                <div>
                  ...
                  <h3 style={{ color: 'var(--foreground)', fontFamily: 'var(--font-display)' }} className="text-base font-semibold leading-snug">{product.name}</h3>
                  <div className="flex gap-3 mt-1">
                    <span style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-body)' }} className="text-xs">Size: {size}</span>
                    <span style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-body)' }} className="text-xs">Colour: {color}</span>
                  </div>
                  <p style={{ color: 'var(--primary)', fontFamily: 'var(--font-display)' }} className="text-sm font-bold mt-1">{fmt(product.price)}</p>
                </div>
                <div className="flex items-center justify-between mt-3">
                  <div style={{ border: '1px solid var(--border)' }} className="flex items-center">
                    <button onClick={() => quantity > 1 ? onUpdateQuantity(product.id, size, color, quantity - 1) : onRemoveItem(product.id, size, color)}
                      style={{ color: 'var(--foreground)' }} className="px-3 py-1.5 hover:bg-white/5 transition-colors focus:outline-none">
                      <Minus size={12} />
                    </button>
                    <span style={{ color: 'var(--foreground)', fontFamily: 'var(--font-display)', borderLeft: '1px solid var(--border)', borderRight: '1px solid var(--border)' }}
                      className="px-4 py-1.5 text-sm min-w-[40px] text-center">{quantity}</span>
                    <button onClick={() => onUpdateQuantity(product.id, size, color, quantity + 1)}
                      style={{ color: 'var(--foreground)' }} className="px-3 py-1.5 hover:bg-white/5 transition-colors focus:outline-none">
                      <Plus size={12} />
                    </button>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => onRemoveItem(product.id, size, color)}
                      style={{ color: 'var(--muted-foreground)' }} className="hover:text-destructive transition-colors focus:outline-none">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
          ...
          <button onClick={() => onNavigate('shop')}
            ...
            className="text-sm hover:text-primary transition-colors focus:outline-none self-start mt-2">
            ← Continue Shopping
          </button>
        </div>
        ...
        <div>
          <div style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }}
            ...
            <h2 style={{ fontFamily: 'var(--font-display)', color: 'var(--foreground)', borderBottom: '1px solid var(--border)' }}
              className="text-lg font-bold uppercase pb-4">Order Summary</h2>
            <div className="flex flex-col gap-3">
              <div className="flex justify-between">
                <span style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-body)' }} className="text-sm">Subtotal</span>
                <span style={{ color: 'var(--foreground)', fontFamily: 'var(--font-display)' }} className="text-sm font-semibold">{fmt(subtotal)}</span>
              </div>
              <div className="flex justify-between">
              ...
              {shipping > 0 && (
                <p style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-body)' }} className="text-xs">
                  Add {fmt(800 - subtotal)} more for free shipping
                </p>
              )}
            </div>
            ...
            <button onClick={() => currentUser ? onNavigate('checkout') : onNavigate('login')}
              style={{ backgroundColor: 'var(--primary)', color: 'var(--primary-foreground)', fontFamily: 'var(--font-display)', letterSpacing: '0.1em' }}
            ...
            {!currentUser && (
              <p style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-body)' }} className="text-xs text-center">Sign in required to checkout</p>
            )}
          </div>
        </div>
      </div>
