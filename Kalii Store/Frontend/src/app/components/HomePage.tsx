...
import { Page, PRODUCTS, CartItem, Currency, CURRENCY_RATES, CURRENCY_SYMBOLS, imgTwoGuys, imgGroupShot } from './types';
...
export function HomePage({ onNavigate, onAddToCart, currency, onGoToProduct }: HomePageProps) {
  ...
  return (
    <div style={{ backgroundColor: 'var(--background)' }}>
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden">
        <div className="absolute inset-0" style={{ backgroundImage: `url(${imgTwoGuys})`, backgroundSize: 'cover', backgroundPosition: 'center top' }} />
        <div className="absolute inset-0" style={{ background: 'linear-gradient(110deg, rgba(8,8,8,0.96) 45%, rgba(8,8,8,0.45) 100%)' }} />
        <div className="relative z-10 max-w-7xl mx-auto px-6 pt-24 pb-16 w-full">
          <div className="max-w-xl">
            <p style={{ color: 'var(--primary)', fontFamily: 'var(--font-body)', letterSpacing: '0.2em' }} className="text-xs uppercase mb-6">New Collection — 2026</p>
            <h1 style={{ fontFamily: 'var(--font-display)', color: 'var(--foreground)', lineHeight: 1.05 }} className="text-6xl md:text-8xl font-bold mb-8 uppercase">
              Inspired By The<br /><span style={{ color: 'var(--primary)' }}>Hustle.</span>
            </h1>
            <p style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-body)', lineHeight: 1.7 }} className="text-base mb-10 max-w-sm">
            ...
            <div className="flex flex-wrap gap-4">
              <button onClick={() => onNavigate('shop')}
                ...
                className="flex items-center gap-2 px-8 py-4 uppercase text-sm font-bold hover:opacity-90 transition-opacity focus:outline-none">
                Shop Now <ArrowRight size={16} />
              </button>
              <button onClick={() => onNavigate('register')}
                style={{ color: 'var(--foreground)', border: '1px solid rgba(255,255,255,0.2)', fontFamily: 'var(--font-display)', letterSpacing: '0.1em' }}
                className="px-8 py-4 uppercase text-sm font-bold hover:border-white/50 transition-colors focus:outline-none">
                Join Kalii
              </button>
            </div>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-20">
        <div className="flex items-end justify-between mb-12">
          ...
          <button onClick={() => onNavigate('shop')} style={{ color: 'var(--primary)', fontFamily: 'var(--font-body)' }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {featured.map(product => (
            <div key={product.id} style={{ backgroundColor: 'var(--card)', border: '1px solid var(--border)' }}
              className="group overflow-hidden cursor-pointer" onClick={() => onGoToProduct(product.id)}>
              <div className="relative overflow-hidden aspect-[3/4] bg-zinc-900">
                ...
                {product.badge && (
                  <span style={{ backgroundColor: 'var(--primary)', color: 'var(--primary-foreground)', fontFamily: 'var(--font-display)' }}
                    className="absolute top-4 left-4 px-3 py-1 text-xs uppercase font-bold tracking-wider">
                    {product.badge}
                  </span>
                )}
                <button
                  onClick={e => { e.stopPropagation(); onAddToCart({ product, quantity: 1, size: product.sizes[0], color: product.colors[0] }); }}
                  style={{ fontFamily: 'var(--font-display)', backgroundColor: 'var(--primary)', color: 'var(--primary-foreground)', letterSpacing: '0.1em' }}
                  className="absolute bottom-4 left-4 right-4 py-3 text-xs uppercase font-bold opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 focus:outline-none">
                  Quick Add
                </button>
              </div>
              <div className="p-4">
                ...
                <p style={{ color: 'var(--primary)', fontFamily: 'var(--font-display)' }} className="text-lg font-bold">{fmt(product.price)}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 text-center md:hidden">
          <button onClick={() => onNavigate('shop')}
            ...
            className="px-8 py-3 text-sm uppercase font-bold focus:outline-none">View All Products</button>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: 'var(--card)', borderTop: '1px solid var(--border)' }} className="py-16">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { name: 'Tops',    image: imgMint1,      count: 8 },
              { name: 'Jackets', image: imgJacketFront, count: 3 },
              { name: 'Sets',    image: imgTracksuit,   count: 2 },
            ].map(cat => (
              <button key={cat.name} onClick={() => onNavigate('shop')}
                className="relative overflow-hidden group focus:outline-none" style={{ aspectRatio: '4/3' }}>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center" style={{ background: 'rgba(8,8,8,0.52)' }}>
      ...
      {/* CTA */}
      <section className="relative py-24 overflow-hidden" style={{ background: 'linear-gradient(135deg, #0f0f0f 0%, #1a1400 100%)' }}>
        ...
        <div className="relative max-w-3xl mx-auto px-6 text-center">
          <p style={{ color: 'var(--primary)', fontFamily: 'var(--font-body)', letterSpacing: '0.2em' }} className="text-xs uppercase mb-4">Members Only</p>
          <h2 style={{ fontFamily: 'var(--font-display)', color: 'var(--foreground)' }} className="text-4xl md:text-5xl font-bold uppercase mb-4">First Order 10% Off</h2>
          <p style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-body)' }} className="text-base mb-8">Create an account and use code <strong style={{ color: 'var(--primary)' }}>HUSTLE10</strong> at checkout. Join the Kalii community today.</p>
          <button onClick={() => onNavigate('register')}
            style={{ backgroundColor: 'var(--primary)', color: 'var(--primary-foreground)', fontFamily: 'var(--font-display)', letterSpacing: '0.1em' }}
            className="px-10 py-4 text-sm uppercase font-bold hover:opacity-90 transition-opacity focus:outline-none">Create Account</button>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            {['Privacy', 'Terms', 'Contact'].map(l => (
              <button key={l} style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-body)' }}
                className="text-xs hover:text-foreground transition-colors focus:outline-none">{l}</button>
            ))}
          </div>
        </div>
      </footer>
